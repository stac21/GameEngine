...
Box::Box(const glm::vec3& center, float depth_x, float depth_y, float depth_z)
	: center(center), depth_x(depth_x), depth_y(depth_y), depth_z(depth_z) {}

Box::Box(const glm::vec3& center, const glm::vec3& depth) :
	center(center), depth(depth) {}

bool Box::isPointInsideBox(const glm::vec3& point) {
